...
echo "$($PSQL "SELECT course FROM courses INNER JOIN majors_courses USING(course_id) INNER JOIN majors USING(major_id) INNER JOIN students USING(major_id) GROUP BY course HAVING COUNT(student_id)=1 ORDER BY course")"